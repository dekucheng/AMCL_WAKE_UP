...
class AmclNode
{
  ...
  private:
    ...
    ros::Publisher cluster_poses_pub_;
  ...
  //beam skip related params
    ...
    // amcl_wakeup customize modification

    double update_timer;
    double update_timer_dt;
};
        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("max_particles", max_particles_, 20000);
  ...
  update_timer = ros::Time::now().toSec();
  double update_timer_dt = 0.0;
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(pf_init_)
  {
    ...
    bool update = fabs(delta.v[0]) > d_thresh_ ||
                  ...
                  fabs(delta.v[2]) > a_thresh_ || update_timer_dt > 0.3;
    update = update || m_force_update;
    m_force_update=false;

    // Update update_timer
    update_timer_dt = ros::Time::now().toSec() - update_timer;
  ...
  // {
  //   std::cout << "!!!!!!!   " << i << "the clusters number is:" << pf_->sets[pf_->current_set].cluster_count << "   !!!!!!" << "x:" << (pf_->sets[pf_->current_set].clusters+i)-> mean.v[0] << "y:" << pf_->sets[pf_->current_set].clusters -> mean.v[1] << "theta:" << pf_->sets[pf_->current_set].clusters -> mean.v[2] <<  std::endl;
  // }
}
